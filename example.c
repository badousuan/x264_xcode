...
int main( int argc, char **argv )
{
    ...
    const char* inPath = "/Users/yuelei/share_for_virtualbox/yuv/352x288.yuv";
    const char* outPath = "/Users/yuelei/Desktop/352x288.h264";
    

    FILE *in  = fopen(inPath, "rb");
    FILE *out  = fopen(outPath, "wb");
    if(!in){
        printf("create in file failed\n");
    }
    if(!out){
...
#define fail fail3
    ...
    for( ;; i_frame++ )
    {
        ...
        {
            if( !fwrite( nal->p_payload, i_frame_size, 1, out ) )
                goto fail;
            else{
                printf("encode 1 frame\n");
            }
        }
    }
